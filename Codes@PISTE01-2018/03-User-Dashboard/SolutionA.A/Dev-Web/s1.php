<?php
$connect = mysqli_connect("localhost", "user", "password", "PISTE01A_2018");
$query = '
SELECT Temperature , 
UNIX_TIMESTAMP(CONCAT_WS(" ", Date, Time)) AS datetime 
FROM Agro_Environmental_Parameters WHERE `Sensor_ID`=2
ORDER BY Date DESC, Time DESC
';
$query1 = '
SELECT Humidity , 
UNIX_TIMESTAMP(CONCAT_WS(" ", Date, Time)) AS datetime 
FROM Agro_Environmental_Parameters WHERE `Sensor_ID`=2 AND `Humidity`<>0
ORDER BY Date DESC, Time DESC
';
$query2 = '
SELECT Luminosity , 
UNIX_TIMESTAMP(CONCAT_WS(" ", Date, Time)) AS datetime 
FROM Agro_Environmental_Parameters WHERE `Sensor_ID`=2 AND `Luminosity`<>0
ORDER BY Date DESC, Time DESC
';
/* $sql = "SELECT * FROM `Agro_Environmental_Parameters` WHERE `ID` = (SELECT max(Row_ID) FROM `Agro_Environmental_Parameters` WHERE `Sensor_ID`=2)"; */
$sql = "SELECT *  FROM `Agro_Environmental_Parameters` WHERE `Sensor_ID` = 2 ORDER BY ROW_ID DESC LIMIT 1";
